...
def run_firewall_scenario(config: ScenarioConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "attempts": attempts,
        "correlation_id": f"bench-{config.name}",
        "retry_count": retry_count,
        "decision": decision,
        "speculative": bool(config.allow_bounded_execution),
        "executed": released,
        "retry_tax_usd": round(cumulative_retry_tax_usd, 6),
        "failure_cost_avoided_usd": estimated_failure_cost_avoided,
        "decision_latency_ms": round(elapsed * 1000.0, 3),
        "reason": None if released else ("terminal_block" if terminal_stop else "retry_tax_block"),
